...
public class MoreBannerFeatures implements ModInitializer {
	...
	static {
		...
		UseEntityCallback.EVENT.register((player, world, hand, entity, hitResult) -> executeDevRenderTools(player, hand) ? ActionResult.SUCCESS : ActionResult.PASS);
		UseItemCallback.EVENT.register((player, world1, hand) -> {
			ItemStack stackInHand = player.getStackInHand(hand);

			return executeDevRenderTools(player, hand) ? TypedActionResult.success(stackInHand) : TypedActionResult.pass(stackInHand);
		});
	}
